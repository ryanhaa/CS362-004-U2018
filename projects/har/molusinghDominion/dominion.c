...
int adventurerEffect(struct gameState *state, int handPos)
{
  ...
  while (drawntreasure <= 2) // BUG: Should be < 2
  {
...
int smithyEffect(struct gameState *state, int handPos)
{
    ...
    int currentPlayer = whoseTurn(state);
    //+3 Cards
    for (i = 0; i <= 3; i++) // bug should be i < 3, will draw 4 cards instead of 3 now
    {
      drawCard(currentPlayer, state);
	printf("Drew Card\n");
    }
...
int remodelEffect(int choice1, int choice2, struct gameState *state, int handPos)
{
  ...
  // BUG: accidently storing in i instead of j in line above, j is uninitialized
...
int villageEffect(struct gameState *state, int handPos)
{
  ...
  //+2 Actions
  state->numActions = state + 2; // BUG: state should be state->numActions
  // this bug is casting the state pointer to an int
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case council_room:
    return council_roomEffect(state, handPos);
