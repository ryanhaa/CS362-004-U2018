...
int main () {
    ...
    for( int i = 0; i < state.deckCount[0]; i++)
    {
      if (state.deck[0][i] != stateOriginal.deck[0][i])
      {
          shuffled = 1;
          shuffledCount++;
      }
    ...
    asserttrue(shuffled,1);

    printf("%s\n","Testing handCount of player" );
    asserttrue(state.handCount[0],stateOriginal.handCount[0]);
    printf("%s\n","Testing numActions" );
    asserttrue(state.numActions,stateOriginal.numActions);
    printf("%s\n","Testing deckCount of player" );
    asserttrue(state.deckCount[0],stateOriginal.deckCount[0]);
    printf("%s\n","Testing playedCardCount of player" );
    asserttrue(state.playedCardCount,state.playedCardCount);
    ...
    printf ("Finished Testing Shuffle.\n\n");
